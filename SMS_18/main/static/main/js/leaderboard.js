...
xhttp.onreadystatechange = function() {
	if (this.readyState == 4 && this.status == 200){
		...
		if ('error' in data){
			...
			for (var i = 0; i < 5; i++) {
				el.innerHTML += '<div class="profileLeader"> <div class="leader"><span>'+ (i+1) +' - </span><span>' + data[i].name + '</span></div></div>';
			...
			if(data[data.length-1].rank>5){
				el.innerHTML += '<div class="profileLeader currentUser"> <div class="leader"><span>'+data[data.length-1].rank+' - </span><span>' + data[data.length-1].name + '</span></div></div>';
